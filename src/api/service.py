...
from src.code_agent.agent import AgentResult, CodeAgent
...
class CodeAgentService:
    ...
    def handle_pr_review(self, repo_full_name: str, issue_number: int, pr_number: int) -> None:
        """
    ...
    def _run_agent_for_issue(
        self, repo_full_name: str, issue_number: int, agent: CodeAgent
    ) -> AgentResult:
    ...
    def _create_and_push_pr(
        ...
        issue_title = github_client.get_issue(repo_full_name, issue_number).title
        commit_message = f"Fix #{issue_number}: {issue_title}\n\nGenerated by Code Agent"
    ...
    def _commit_pr_changes(self, pr_number: int, agent: CodeAgent, result: AgentResult) -> None:
